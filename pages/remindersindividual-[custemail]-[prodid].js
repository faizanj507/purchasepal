...
import { GETALLREMINDERSBYCUSTOMERS } from '../fetchAPI';
import Link from 'next/link';
import { MobileBackArrowMajor } from '@shopify/polaris-icons';

export default function RemindersIndividual() {
    let [prodcustomer,setprodcustomer] = useState([]);
    let router = useRouter();
    let email = router.asPath.split("-")[1];
    let pid = router.asPath.split("-")[2];
    
    useEffect(()=>{
      GETALLREMINDERSBYCUSTOMERS({email:email,pid:pid}).then(res=>{
        res.json().then(res=>{
          setprodcustomer(res)
        })
      });
    ...
    return (
        <Page title={<><Button plain icon={MobileBackArrowMajor} onClick={()=>{router.back()}} ></Button> Cutomer Reminders</>}>
            <Card sectioned>
              <DataTable
                columnContentTypes={[
                  'text',
